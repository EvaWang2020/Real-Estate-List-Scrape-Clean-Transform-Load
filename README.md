...
Repo main documents:
- realestatelist1.py
  - It is a Python script to scrape real estate lists of multiple cities. However, the website that I used only **allows to scrape at most 25 pages for each city**, so some lists from the cities are missing. If you want to scrape the whole list of each city in Vancouver, check [my article](https://evaanalytics.wixsite.com/website/post/use-scrapy-to-real-estate-data), which uses another website as an example. However, it will **not allow you to scrape land size for houses**. But, it allows you to scrape list type if you change the URL format and add list type parameters inside.
